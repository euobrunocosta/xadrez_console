	...
	class PartidaDeXadrez {
		...
		public void realizaJogada(Posicao origem, Posicao destino) {
			...
			if (estaEmXeque(jogadorAtual)) {
				...
				throw new TabuleiroException("Você não pode se colocar me cheque");
			}
		...
		public void validarPosicaoDeOrigem(Posicao pos) {
			if (tab.peca(pos) == null) {
				throw new TabuleiroException("Não existem peça a posição de origem");
			}
			if (jogadorAtual != tab.peca(pos).cor) {
				throw new TabuleiroException("A peça de origem escolhida não é sua");
			}
			if (!tab.peca(pos).existemMovimentosPossiveis()) {
				throw new TabuleiroException("Não há movimentos possíveis apra a peça escolhida");
			}
		}
